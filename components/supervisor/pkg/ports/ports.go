...
func (pm *Manager) nextState() map[uint32]*managedPort {
	...
	// 3. stop proxies for closed localhost ports
	for _, proxy := range pm.proxies {
		...
		delete(pm.proxies, proxy.localPort)
		delete(pm.internal, proxy.globalPort)
	}

	// 4. capture served ports, start localhost proxies and auto expose ports as needed
	for _, served := range pm.served {
		...
		if served.BoundToLocalhost {
			...
			if !exists {
				newProxy, err := pm.proxyStarter(port, opened)
				if err != nil {
					log.WithError(err).WithField("port", served.Port).Warn("cannot start localhost proxy")
				} else {
					opened[newProxy.globalPort] = struct{}{}
					pm.internal[newProxy.globalPort] = struct{}{}
					pm.served = append(pm.served, ServedPort{Port: newProxy.globalPort, BoundToLocalhost: false})
		...
		var config *gitpod.PortConfig
		if pm.configs != nil {
			config, _ = pm.configs.Get(mp.LocalhostPort)
		}
		pm.autoExpose(ctx, mp, config)
	}
	return state
